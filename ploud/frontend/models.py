...
class User(ptah.get_base()):
    ...
    @classmethod
    def get(self, email):
        return ptah.get_session().query(User).filter_by(email=email).first()

    @classmethod
    def getById(self, id):
        return ptah.get_session().query(User).filter_by(id=id).first()
...
class WaitingList(ptah.get_base()):
    ...
    def __init__(self, email, ipaddr=None):
        super(WaitingList, self).__init__()
        self.email = email
        self.ipaddr = ipaddr
        self.requested = datetime.datetime.now()


class Error(ptah.get_base()):
    """ Log traceback """
